...
##'
##' [aqs_qablanks] sends a request to the AQS API based on a user-provided
##' filter using the following underlying functions:
##' - [qablanks_bysite] returns `param` blank data for `site` in `county`,
##' within `state`, between `bdate` and `edate`.
##' - [qablanks_bycounty] returns `param` blank data for `county` in `state`
##' between `bdate` and `edate`.
##' - [qablanks_bystate] returns `param` blank data for `state` between `bdate`
##' and `edate`.
##' - [qablanks_bypqao] returns `param` blank data for `pqao` between `bdate`
##' and `edate`.
##' - [qablanks_byma] returns `param` blank data for `agency` (monitoring
##' agency) between `bdate` and `edate`.
##'
...
##'
##' @export
aqs_qablanks <- function(aqs_filter = c("bySite", "byCounty", "byState",
                                        "byPQAO", "byMA"),
  ...
  switch(
    aqs_filter,
    bySite = do.call(qablanks_bysite, aqs_variables),
    byCounty = do.call(qablanks_bycounty, aqs_variables),
    byState = do.call(qablanks_bystate, aqs_variables),
    byPQAO = do.call(qablanks_bypqao, aqs_variables),
    byMA = do.call(qablanks_byma, aqs_variables)
  )
}

##' @rdname aqs_qablanks
##' @export
qablanks_bysite <- function(param, bdate, edate, state, county, site,
                            email = get_aqs_email(), key = get_aqs_key(),
                            header = FALSE, ...) {
  .run_api(
    aqs_service = "qaBlanks", aqs_filter = "bySite",
...
##' @rdname aqs_qablanks
##' @export
qablanks_bycounty <- function(param, bdate, edate, state, county,
                              email = get_aqs_email(), key = get_aqs_key(),
                              header = FALSE, ...) {
  .run_api(
    aqs_service = "qaBlanks", aqs_filter = "byCounty",
    aqs_variables = list(email = email, key = key, param = param,
                         bdate = bdate, edate = edate,
...
##' @rdname aqs_qablanks
##' @export
qablanks_bystate <- function(param, bdate, edate, state,
                             email = get_aqs_email(), key = get_aqs_key(),
                             header = FALSE, ...) {
  .run_api(
    aqs_service = "qaBlanks", aqs_filter = "byState",
    aqs_variables = list(email = email, key = key, param = param,
...
##' @rdname aqs_qablanks
##' @export
qablanks_bypqao <- function(param, bdate, edate, pqao,
                            email = get_aqs_email(), key = get_aqs_key(),
                            header = FALSE, ...) {
  .run_api(
    aqs_service = "qaBlanks", aqs_filter = "byPQAO",
    aqs_variables = list(email = email, key = key, param = param,
...
##' @rdname aqs_qablanks
##' @export
qablanks_byma <- function(param, bdate, edate, agency,
                          email = get_aqs_email(), key = get_aqs_key(),
                          header = FALSE, ...) {
  .run_api(
    aqs_service = "qaBlanks", aqs_filter = "byMA",
    aqs_variables = list(email = email, key = key, param = param,
