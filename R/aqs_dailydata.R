...
##' request size.
##'
##' [aqs_dailydata] sends a request to the AQS API based on a user-provided
##' filter using the following underlying functions:
##' - [dailydata_bysite] returns daily summary `param` data for `site` in
##' `county`, within `state`, between `bdate` and `edate`.
##' - [dailydata_bycounty] returns daily summary `param` data for `county` in
##' `state` between `bdate` and `edate`.
##' - [dailydata_bystate] returns daily summary `param` data for `state` between
##' `bdate` and `edate`.
##' - [dailydata_bybox] returns daily summary `param` data for a user-provided
##' latitude/longitude bounding box (`minlat`, `maxlat`, `minlon`, `maxlon`)
##' between `bdate` and `edate`.
##' - [dailydata_bycbsa] returns daily summary `param` data for a user-provided
##' CBSA between `bdate` and `edate`.
##'
...
##'
##' @export
aqs_dailydata <- function(aqs_filter = c("bySite", "byCounty", "byState",
                                          "byBox", "byCBSA"),
  ...
  switch(
    aqs_filter,
    bySite = do.call(dailydata_bysite, aqs_variables),
    byCounty = do.call(dailydata_bycounty, aqs_variables),
    byState = do.call(dailydata_bystate, aqs_variables),
    byBox = do.call(dailydata_bybox, aqs_variables),
    byCBSA = do.call(dailydata_bycbsa, aqs_variables)
  )
}

##' @rdname aqs_dailydata
##' @export
dailydata_bysite <- function(param, bdate, edate, state, county, site,
                              email = get_aqs_email(), key = get_aqs_key(),
                              duration = NULL, cbdate = NULL, cedate = NULL,
                              header = FALSE, ...) {
  .run_api(
    aqs_service = "dailyData", aqs_filter = "bySite",
...
##' @rdname aqs_dailydata
##' @export
dailydata_bycounty <- function(param, bdate, edate, state, county,
                                email = get_aqs_email(), key = get_aqs_key(),
                                duration = NULL, cbdate = NULL, cedate = NULL,
                                header = FALSE, ...) {
  .run_api(
    aqs_service = "dailyData", aqs_filter = "byCounty",
    aqs_variables = list(email = email, key = key, param = param,
                         bdate = bdate, edate = edate,
...
##' @rdname aqs_dailydata
##' @export
dailydata_bystate <- function(param, bdate, edate, state,
                               email = get_aqs_email(), key = get_aqs_key(),
                               duration = NULL, cbdate = NULL, cedate = NULL,
                               header = FALSE, ...) {
  .run_api(
    aqs_service = "dailyData", aqs_filter = "byState",
    aqs_variables = list(email = email, key = key, param = param,
...
##' @rdname aqs_dailydata
##' @export
dailydata_bybox <- function(param, bdate, edate,
                             minlat, maxlat, minlon, maxlon,
                             email = get_aqs_email(), key = get_aqs_key(),
                             duration = NULL, cbdate = NULL, cedate = NULL,
                             header = FALSE, ...) {
  .run_api(
    aqs_service = "dailyData", aqs_filter = "byBox",
    aqs_variables = list(email = email, key = key, param = param,
                         bdate = bdate, edate = edate,
...
##' @rdname aqs_dailydata
##' @export
dailydata_bycbsa <- function(param, bdate, edate, cbsa,
                              email = get_aqs_email(), key = get_aqs_key(),
                              duration = NULL, cbdate = NULL, cedate = NULL,
                              header = FALSE, ...) {
  .run_api(
    aqs_service = "dailyData", aqs_filter = "byCBSA",
    aqs_variables = list(email = email, key = key, param = param,
