...
##'
##' A collection of functions to fetch pairs of PM samples collected at the same
##' time and place by different samplers.
##'
##' [aqs_qacollocatedassessments] sends a request to the AQS API based on a
##' user-provided filter using the following underlying functions:
##' - [qacollocatedassessments_bysite] returns collocated assessment data for
##' `param` at `site` in `county`, within `state`, between `bdate` and `edate`.
##' - [qacollocatedassessments_bycounty] returns collocated assessment data for
##' `param` in `county` within `state` between `bdate` and `edate`.
##' - [qacollocatedassessments_bystate] returns collocated assessment data for
##' `param` in `state` between `bdate` and `edate`.
##' - [qacollocatedassessments_bypqao] returns collocated assessment data for
##' `param` in `pqao` between `bdate` and `edate`.
##' - [qacollocatedassessments_byma] returns collocated assessment data for
...
##'
##' @export
aqs_qacollocatedassessments <- function(aqs_filter = c("bySite", "byCounty",
                                                       "byState", "byPQAO",
                                                       "byMA"),
                                        aqs_variables = NULL,
                                        header = FALSE, ...) {
  ...
  switch(
    aqs_filter,
    bySite = do.call(qacollocatedassessments_bysite, aqs_variables),
    byCounty = do.call(qacollocatedassessments_bycounty, aqs_variables),
    byState = do.call(qacollocatedassessments_bystate, aqs_variables),
    byPQAO = do.call(qacollocatedassessments_bypqao, aqs_variables),
    byMA = do.call(qacollocatedassessments_byma, aqs_variables)
  )
}
...
qacollocatedassessments_bysite <- function(param, bdate, edate, state,
                                           ...
                                           header = FALSE, ...) {
  .run_api(
    aqs_service = "qaCollocatedAssessments", aqs_filter = "bySite",
...
qacollocatedassessments_bycounty <- function(param, bdate, edate, state, county,
                                             ...
                                             header = FALSE, ...) {
  .run_api(
    aqs_service = "qaCollocatedAssessments", aqs_filter = "byCounty",
    aqs_variables = list(email = email, key = key, param = param,
                         bdate = bdate, edate = edate,
...
qacollocatedassessments_bystate <- function(param, bdate, edate, state,
                                            ...
                                            header = FALSE, ...) {
  .run_api(
    aqs_service = "qaCollocatedAssessments", aqs_filter = "byState",
    aqs_variables = list(email = email, key = key, param = param,
...
qacollocatedassessments_bypqao <- function(param, bdate, edate, pqao,
                                           ...
                                           header = FALSE, ...) {
  .run_api(
    aqs_service = "qaCollocatedAssessments", aqs_filter = "byPQAO",
    aqs_variables = list(email = email, key = key, param = param,
...
qacollocatedassessments_byma <- function(param, bdate, edate, agency,
                                         ...
                                         header = FALSE, ...) {
  .run_api(
    aqs_service = "qaCollocatedAssessments", aqs_filter = "byMA",
    aqs_variables = list(email = email, key = key, param = param,
