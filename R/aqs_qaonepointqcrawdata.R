...
##'
##' A collection of functions to fetch measured versus actual concentration of 1
##' point QC checks.
##'
##' [aqs_qaonepointqcrawdata] sends a request to the AQS API based on a
##' user-provided filter using the following underlying functions:
##' - [qaonepointqcrawdata_bysite] returns One Point QC data for `param` at
##' `site` in `county`, within `state`, between `bdate` and `edate`.
##' - [qaonepointqcrawdata_bycounty] returns One Point QC data for `param` in
##' `county` within `state` between `bdate` and `edate`.
##' - [qaonepointqcrawdata_bystate] returns One Point QC data for `param` in
##' `state` between `bdate` and `edate`.
##' - [qaonepointqcrawdata_bypqao] returns One Point QC data for `param` in
##' `pqao` between `bdate` and `edate`.
##' - [qaflowrateaudits_byma] returns One Point QC data for `param` in `agency`
...
##'
##' @export
aqs_qaonepointqcrawdata <- function(aqs_filter = c("bySite", "byCounty",
                                                   "byState", "byPQAO",
                                                   "byMA"),
                                    aqs_variables = NULL,
                                    header = FALSE, ...) {
  ...
  switch(
    aqs_filter,
    bySite = do.call(qaonepointqcrawdata_bysite, aqs_variables),
    byCounty = do.call(qaonepointqcrawdata_bycounty, aqs_variables),
    byState = do.call(qaonepointqcrawdata_bystate, aqs_variables),
    byPQAO = do.call(qaonepointqcrawdata_bypqao, aqs_variables),
    byMA = do.call(qaonepointqcrawdata_byma, aqs_variables)
  )
}

##' @rdname aqs_qaonepointqcrawdata
##' @export
qaonepointqcrawdata_bysite <- function(param, bdate, edate, state, county, site,
                                       email = get_aqs_email(),
                                       key = get_aqs_key(),
                                       header = FALSE, ...) {
  .run_api(
    aqs_service = "qaOnePointQcRawData", aqs_filter = "bySite",
...
##' @rdname aqs_qaonepointqcrawdata
##' @export
qaonepointqcrawdata_bycounty <- function(param, bdate, edate, state, county,
                                         email = get_aqs_email(),
                                         key = get_aqs_key(),
                                         header = FALSE, ...) {
  .run_api(
    aqs_service = "qaOnePointQcRawData", aqs_filter = "byCounty",
    aqs_variables = list(email = email, key = key, param = param,
                         bdate = bdate, edate = edate,
...
##' @rdname aqs_qaonepointqcrawdata
##' @export
qaonepointqcrawdata_bystate <- function(param, bdate, edate, state,
                                        email = get_aqs_email(),
                                        key = get_aqs_key(),
                                        header = FALSE, ...) {
  .run_api(
    aqs_service = "qaOnePointQcRawData", aqs_filter = "byState",
    aqs_variables = list(email = email, key = key, param = param,
...
##' @rdname aqs_qaonepointqcrawdata
##' @export
qaonepointqcrawdata_bypqao <- function(param, bdate, edate, pqao,
                                       email = get_aqs_email(),
                                       key = get_aqs_key(),
                                       header = FALSE, ...) {
  .run_api(
    aqs_service = "qaOnePointQcRawData", aqs_filter = "byPQAO",
    aqs_variables = list(email = email, key = key, param = param,
...
##' @rdname aqs_qaonepointqcrawdata
##' @export
qaonepointqcrawdata_byma <- function(param, bdate, edate, agency,
                                     email = get_aqs_email(),
                                     key = get_aqs_key(),
                                     header = FALSE, ...) {
  .run_api(
    aqs_service = "qaOnePointQcRawData", aqs_filter = "byMA",
    aqs_variables = list(email = email, key = key, param = param,
