...
##'
##' A collection of functions to fetch flow rate checks performed by
##' monitoring agencies.
##'
##' [aqs_qaflowrateverifications] sends a request to the AQS API based on a
##' user-provided filter using the following underlying functions:
##' - [qaflowrateverifications_bysite] returns Flow Rate Verification data for
##' `param` at `site` in `county`, within `state`, between `bdate` and `edate`.
##' - [qaflowrateverifications_bycounty] returns Flow Rate Verification data for
##' `param` in `county` within `state` between `bdate` and `edate`.
##' - [qaflowrateverifications_bystate] returns Flow Rate Verification data for
##' `param` in `state` between `bdate` and `edate`.
##' - [qaflowrateverifications_bypqao] returns Flow Rate Verification data for
##' `param` in `pqao` between `bdate` and `edate`.
##' - [qaflowrateverifications_byma] returns Flow Rate Verification data for
##' `param` in `agency` (monitoring agency) between `bdate` and `edate`.
##'
...
##'
##' @export
aqs_qaflowrateverifications <- function(aqs_filter = c("bySite", "byCounty",
                                                       "byState", "byPQAO",
                                                       "byMA"),
                                        aqs_variables = NULL,
                                        header = FALSE, ...) {
  ...
  switch(
    aqs_filter,
    bySite = do.call(qaflowrateverifications_bysite, aqs_variables),
    byCounty = do.call(qaflowrateverifications_bycounty, aqs_variables),
    byState = do.call(qaflowrateverifications_bystate, aqs_variables),
    byPQAO = do.call(qaflowrateverifications_bypqao, aqs_variables),
    byMA = do.call(qaflowrateverifications_byma, aqs_variables)
  )
}

##' @rdname aqs_qaflowrateverifications
##' @export
qaflowrateverifications_bysite <- function(param, bdate, edate, state,
                                           county, site,
                                           email = get_aqs_email(),
                                           key = get_aqs_key(),
                                           header = FALSE, ...) {
  .run_api(
    aqs_service = "qaFlowRateVerifications", aqs_filter = "bySite",
...
##' @rdname aqs_qaflowrateverifications
##' @export
qaflowrateverifications_bycounty <- function(param, bdate, edate, state, county,
                                             email = get_aqs_email(),
                                             key = get_aqs_key(),
                                             header = FALSE, ...) {
  .run_api(
    aqs_service = "qaFlowRateVerifications", aqs_filter = "byCounty",
    aqs_variables = list(email = email, key = key, param = param,
                         bdate = bdate, edate = edate,
...
##' @rdname aqs_qaflowrateverifications
##' @export
qaflowrateverifications_bystate <- function(param, bdate, edate, state,
                                            email = get_aqs_email(),
                                            key = get_aqs_key(),
                                            header = FALSE, ...) {
  .run_api(
    aqs_service = "qaFlowRateVerifications", aqs_filter = "byState",
    aqs_variables = list(email = email, key = key, param = param,
...
##' @rdname aqs_qaflowrateverifications
##' @export
qaflowrateverifications_bypqao <- function(param, bdate, edate, pqao,
                                           email = get_aqs_email(),
                                           key = get_aqs_key(),
                                           header = FALSE, ...) {
  .run_api(
    aqs_service = "qaFlowRateVerifications", aqs_filter = "byPQAO",
    aqs_variables = list(email = email, key = key, param = param,
...
##' @rdname aqs_qaflowrateverifications
##' @export
qaflowrateverifications_byma <- function(param, bdate, edate, agency,
                                         email = get_aqs_email(),
                                         key = get_aqs_key(),
                                         header = FALSE, ...) {
  .run_api(
    aqs_service = "qaFlowRateVerifications", aqs_filter = "byMA",
    aqs_variables = list(email = email, key = key, param = param,
