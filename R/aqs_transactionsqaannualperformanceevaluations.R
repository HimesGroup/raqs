...
##'
##' A collection of functions to fetch pairs of data QA at several concentration
##' levels in the submission (transaction) format for AQS.
##'
##' [aqs_transactionsqaannualperformanceevaluations] sends a request to the AQS
##' API based on a user-provided filter using the following underlying
##' functions:
##' - [transactionsqaannualperformanceevaluations_bysite] returns annual
##' performance evaluation data for `param` at `site` in `county`, within
##' `state`, between `bdate` and `edate` in the transaction format.
##' - [transactionsqaannualperformanceevaluations_bycounty] returns annual
##' performance evaluation data for `param` in `county` within `state` between
##' `bdate` and `edate` in the transaction format.
##' - [transactionsqaannualperformanceevaluations_bystate] returns annual
##' performance evaluation data for `param` in `state` between `bdate` and
##' `edate` in the transaction format.
##' - [transactionsqaannualperformanceevaluations_bypqao] returns annual
##' performance evaluation data for `param` in `pqao` between `bdate` and
##' `edate` in the transaction format.
##' - [transactionsqaannualperformanceevaluations_byma] returns annual
##' performance evaluation data for `param` in `agency` (monitoring agency)
##' between `bdate` and `edate` in the transaction format.
##'
...
##'
##' @export
aqs_transactionsqaannualperformanceevaluations <- function(aqs_filter = c(
                                                             "bySite",
                                                             "byCounty",
                                                             "byState",
                                                             "byPQAO",
  ...
  switch(
    aqs_filter,
    bySite = do.call(transactionsqaannualperformanceevaluations_bysite,
                     aqs_variables),
    byCounty = do.call(transactionsqaannualperformanceevaluations_bycounty,
                       aqs_variables),
    byState = do.call(transactionsqaannualperformanceevaluations_bystate,
                      aqs_variables),
    byPQAO = do.call(transactionsqaannualperformanceevaluations_bypqao,
                     aqs_variables),
    byMA = do.call(transactionsqaannualperformanceevaluations_byma,
                   aqs_variables)
  )
}

##' @rdname aqs_transactionsqaannualperformanceevaluations
##' @export
transactionsqaannualperformanceevaluations_bysite <- function(param, bdate,
                                                              edate, state,
                                                              county, site,
                                                              email = get_aqs_email(),
                                                              key = get_aqs_key(),
                                                              ...
                                                              ...) {
  .run_api(
    aqs_service = "transactionsQaAnnualPerformanceEvaluations",
    aqs_filter = "bySite",
...
##' @rdname aqs_transactionsqaannualperformanceevaluations
##' @export
transactionsqaannualperformanceevaluations_bycounty <- function(param, bdate,
                                                                edate, state,
                                                                county,
                                                                email = get_aqs_email(),
                                                                key = get_aqs_key(),
...
##' @rdname aqs_transactionsqaannualperformanceevaluations
##' @export
transactionsqaannualperformanceevaluations_bystate <- function(param, bdate,
                                                               edate, state,
                                                               email = get_aqs_email(),
                                                               key = get_aqs_key(),
                                                               header = FALSE,
...
##' @rdname aqs_transactionsqaannualperformanceevaluations
##' @export
transactionsqaannualperformanceevaluations_bypqao <- function(param, bdate,
                                                              edate, pqao,
                                                              email = get_aqs_email(),
                                                              key = get_aqs_key(),
                                                              header = FALSE,
...
##' @rdname aqs_transactionsqaannualperformanceevaluations
##' @export
transactionsqaannualperformanceevaluations_byma <- function(param, bdate,
                                                            edate, agency,
                                                            email = get_aqs_email(),
                                                            key = get_aqs_key(),
                                                            header = FALSE,
